...
export const validateUserCreate = (req, res, next) => {
    ...
    // ----- EMAIL VALIDATION -----
    //
    // [[Email Test:FALSY:mw]] Email is falsy
    if (!email) {
        issues.push({
    ...
    // [[Email Test:TYPE:mw]] Email is not string
    if (email && 
        typeof email !== 'string') {
        //
        issues.push({
    ...
    // [[Email Test:LENGTH:mw]] Email length out of limits
    ...
    // [[Email Test:FORMAT:mw]] Is OK format [[!mark]]
    if (email && typeof email === 'string'
        && !isEmail(email)) {
        //
        issues.push({
    ...
    //
    // [[Password Hash Test:FALSY:mw]] Password is falsy
    if (!password) {
        issues.push({
    ...
    // [[Password Hash Test:TYPE:mw]] Is password hash not a string
    if (password
    ...
    // [[Password Hash Test:LENGTH:mw]] Pwd hash incorrect length
    if (password
        && typeof password === 'string'
